...
(function () {
  ...
  /** Filtra la lista por pestaña y texto. Se prueba automáticamente. */
  function filtrar(lista, filtro, texto) {
    var t = plano(texto).trim();
    return lista.filter(function (r) {
      var pasaFiltro =
        filtro === 'TODOS' ? true :
        filtro === 'GANADOR' ? r.ganador :
        filtro === 'INSCRITO' || filtro === 'ACTIVO' ? r.estado === filtro :
        r.categoria === filtro;
  ...
  function pintarTodo() {
    ...
    $('nTotal').textContent = S.resumen.total || 0;
    $('nInscritos').textContent = S.resumen.inscritos || 0;
    $('nActivos').textContent = S.resumen.activos || 0;
    $('nGanadores').textContent = S.resumen.ganadores || 0;
    Array.prototype.forEach.call(document.querySelectorAll('.dato'), function (d) {
      d.classList.toggle('sel', d.dataset.filtro === S.filtro);
    });
    pintarFiltros();
    pintarLista();
  }
  ...
  function pintarFiltros() {
    var cont = $('filtros');
    var firma = S.categorias.join('|') + '::' + S.filtro;
    if (cont.dataset.firma !== firma) {
      cont.dataset.firma = firma;
      cont.innerHTML = '';
      var todos = document.createElement('button');
      todos.className = 'filtro' + (S.filtro === 'TODOS' ? ' sel' : '');
      todos.textContent = 'Todas las categorías';
      todos.addEventListener('click', function () { S.filtro = 'TODOS'; pintarTodo(); });
      cont.appendChild(todos);

      S.categorias.forEach(function (c, i) {
        var b = document.createElement('button');
        b.className = 'filtro' + (S.filtro === c ? ' sel' : '');
        var color = ['var(--coral)', 'var(--sol)', 'var(--agua)', 'var(--verde-2)', 'var(--lila)'][i % 5];
        b.innerHTML = '<i style="background:' + color + '"></i>' + esc(c);
        b.addEventListener('click', function () { S.filtro = c; pintarTodo(); });
        cont.appendChild(b);
      });
    }
  }
  ...
  function pintarLista() {
    var vista = filtrar(S.lista, S.filtro, S.texto);
    var cont = $('lista');
    cont.innerHTML = '';
    $('conteoVista').textContent = vista.length + (vista.length === 1 ? ' registro' : ' registros') +
      (S.filtro === 'TODOS' ? '' : ' · ' + (S.filtro === 'GANADOR' ? 'ganadores' : S.filtro.toLowerCase()));
  ...
  function filasPdf(lista, tipo) {
    return filtrar(lista, tipo, '').map(function (r, i) {
  ...
  function arrancar() {
    ...
    Array.prototype.forEach.call(document.querySelectorAll('.dato'), function (d) {
      d.addEventListener('click', function () { S.filtro = d.dataset.filtro; pintarTodo(); });
    });
  ...
  window.ADM = { filtrar: filtrar, filasPdf: filasPdf, plano: plano, valDocumento: valDocumento,
